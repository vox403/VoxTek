...
<head>
  <meta charset="UTF-8">
  <title>VoxTek Emotion Firewall</title>
  <style>
    body {
    ...
    h1 {
      ...
      margin-bottom: 16px;
      color: #46b9ff;
      text-shadow: 0 0 6px rgba(70, 185, 255, 0.8);
    }
    ...
    input:focus {
      border-color: #46b9ff;
      box-shadow: 0 0 6px rgba(70, 185, 255, 0.7);
    }
    ...
    .card.vox-mode {
      border-color: #2ca8ff;
      box-shadow: 0 0 18px rgba(44, 168, 255, 0.6);
    }

    /* 알래스터 관련 감정 모드 (레드) */
    .card.alastor-mode {
      border-color: #ff3150;
      box-shadow: 0 0 20px rgba(255, 49, 80, 0.75);
    }
...
<body>
  ...
  <div class="panel">
    <div class="controls">
      <input id="emotionInput" placeholder="감정을 입력하세요 (예: 사랑, 그리움, 분노, 불안, 오만, 열등감, 알래스터)">
...
대기 중...
  ...
  <script>
    ...
    function triggerEmotion() {
      ...
      // 기본 모드 초기화
      logBox.classList.remove("vox-mode", "alastor-mode");
      ...
      if (type) {
        ...
        // 카드 컬러 모드
        if (isAlastor) {
          logBox.classList.add("alastor-mode");
        } else {
...
------------------------------------------------------------`;
        ...
        // 제한되지 않은 감정 (200 OK)
        logBox.textContent =
`[200 OK] Emotion '${emotion || "NULL"}' processed successfully.
